...
- A: express.Router() is used to create modular application that allows developers to seperate routes in seperate set of files.This gives cleaness to the overall project and easy expandability.
...
--- A: To handle errors in the Express we first need to write an error number within res.status() and .send('Error message here') to indicate any error that was caught which correnspond to the error code. EXAMPLE:
...
- A: This line allows Node.JS code runs on a dynamic port that it has defined. Local development port is 8081
